...
async def test_ask_concatenates_streamed_text_and_scopes_cwd(tmp_path):
    ...
    with (
        ...
        patch("coderag_mcp.orchestrator.ask.clone.clone_repo", return_value=tmp_path) as mock_clone_repo,
        patch("coderag_mcp.orchestrator.ask.clone.cleanup_clone") as mock_cleanup_clone,
    ...
    mock_clone_repo.assert_called_once_with("https://github.com/a/b", allow_local_paths=False)
    mock_cleanup_clone.assert_called_once_with(tmp_path)
